...
func SearchReceipt(client *ethclient.Client) {
	...
	for _, receipt := range receiptByHash {
		...
		fmt.Println("hash", receipt.TxHash.Hex())  // 0x20294a03e8766e9aeab58327fc4112756017c6c28f6f99c7722f4a29075601c5
	...
	for _, receipt := range receiptsByNum {
		...
		fmt.Println("logs", receipt.Logs)                     // []
		fmt.Println("hash", receipt.TxHash.Hex())             // 0x20294a03e8766e9aeab58327fc4112756017c6c28f6f99c7722f4a29075601c5
		fmt.Println("index:", receipt.TransactionIndex)       // 0
		fmt.Println("address", receipt.ContractAddress.Hex()) // 0x0000000000000000000000000000000000000000
		break
	}
