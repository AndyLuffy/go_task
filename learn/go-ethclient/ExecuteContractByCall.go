...
func ExecuteContractByCall(client *ethclient.Client, privateKey *ecdsa.PrivateKey) {
	...
	*/
	...
	var key [32]byte
	var value [32]byte
	copy(key[:], []byte("demo_save_key_no_use_abi789"))
	copy(value[:], []byte("demo_save_value_no_use_abi_11111999"))
	...
	_, err = waitForReceipt3(client, signedTx.Hash())
	if err != nil {
		log.Fatal(err)
	}
